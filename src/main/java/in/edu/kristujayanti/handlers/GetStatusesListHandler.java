...
public class GetStatusesListHandler implements Handler<RoutingContext> {
  private final AssetsService assetsService;
  public GetStatusesListHandler(AssetsService assetsService) { this.assetsService = assetsService; }
  @Override
  public void handle(RoutingContext routingContext) {
    HttpServerResponse response = routingContext.response();
    try {
      JsonArray result = assetsService.getDistinctStatuses();
      ResponseUtil.createResponse(response, ResponseType.SUCCESS, StatusCode.TWOHUNDRED,
        new JsonObject().put("statuses", result), new JsonArray());
    } catch (Exception e) {
